...
**TTM** is a real-time task management application for game development teams with hierarchical structure (Projects → Milestones → Tasks), collaborative features, and CSV import capabilities.
...
```
src/lib/services/
├── ProjectService.ts      # Projects, members, invites
├── MilestoneService.ts    # Milestones
├── TaskService.ts         # Tasks
├── UserService.ts         # User profiles, email lookup, photo upload
├── CsvFormatService.ts    # CSV format templates
├── ProjectPostService.ts  # Project board posts
├── ScrumService.ts        # Daily scrum entries
...
```
src/lib/hooks/
├── useProjects.ts         # User's projects (real-time)
├── useProject.ts          # Single project
├── useMilestones.ts       # Project's milestones
├── useMilestone.ts        # Single milestone
├── useProjectPosts.ts     # Project board posts
├── useScrums.ts           # Project's scrums (real-time)
├── useTasks.ts            # Milestone's tasks (real-time)
└── useMembers.ts          # Member profiles batch fetch
```
...
├── projects/[projectId]/milestones/page.tsx    # Milestones + board + calendar
├── projects/[projectId]/milestones/[milestoneId]/page.tsx  # Tasks (board/table)
└── projects/[projectId]/milestones/scrum/page.tsx          # Daily scrum
```

### API Routes
```
src/app/api/
└── discord-notify/route.ts   # Server-side Discord webhook proxy
```

### Components (Atomic Design)
```
src/components/
├── atoms/           # Basic UI: Button, Card, Chip, Avatar, InputField, etc.
├── molecules/       # FormField, MarkdownEditorPanel, ThemeToggle
├── organisms/       # Feature components (see breakdown below)
...
// Members
inviteMemberByEmail(projectId, email)  // Find by email → add to project
...
moveTask(projectId, fromMilestoneId, toMilestoneId, taskId, taskData)  // Move between milestones
```
...
### ProjectPostService ([ProjectPostService.ts](src/lib/services/ProjectPostService.ts))

```typescript
subscribePosts(projectId, callback)

createPost(projectId, {title, content, category, authorId, authorName, authorPhotoURL})
updatePost(projectId, postId, {title?, content?, category?})
deletePost(projectId, postId)
```

**Firestore Path:** `projects/{projectId}/posts`

---

### DiscordService ([DiscordService.ts](src/lib/services/DiscordService.ts))

```typescript
notifyProjectCreated(userName, projectName)
notifyMilestoneCreated(userName, projectName, milestoneName)
notifyTaskUpdated(userName, taskName, details?)
// ... other activity notifications
```

**API Route:** `src/app/api/discord-notify/route.ts`
**Env:** `DISCORD_WEBHOOK_URL` (server-side)
...
- **[ProjectBoard.tsx](src/components/organisms/projects/ProjectBoard.tsx)** - Toggleable project board with posts, category filters (with visual highlighting), and animated toggle interactions
...
- **[MilestoneHeader.tsx](src/components/organisms/milestones/MilestoneHeader.tsx)** - Title + actions + CSV export
...
- **[TaskDetailsPanel.tsx](src/components/organisms/tasks/TaskDetailsPanel.tsx)** - Edit drawer (markdown support, move to other milestones)
...
## 📊 Database Schema (Firestore + Storage)
...
users/{uid}/csvFormats/{formatId}  📎 Subcollection
  ├── name: string
  ├── hasHeader: boolean
  ├── columns: {scene?, category?, feature?, detail?, logic?, progress?, result?}
  ├── ownerId: string
...
projects/{projectId}
  ...
  ├── ownerId: string
  ├── memberIds: string[]
  ├── memberRoles: {[uid]: "owner"|"admin"|"editor"|"viewer"}
  ├── createdAt: Timestamp
  └── updatedAt: Timestamp

projects/{projectId}/milestones/{milestoneId}  📎 Subcollection
  ├── title: string
  ├── description: string
...
projects/{projectId}/milestones/{milestoneId}/tasks/{taskId}  📎 Subcollection
  ├── title: string
...
projects/{projectId}/posts/{postId}  📎 Subcollection
  ├── title: string
...
profile-photos/{uid}/avatar.{ext}
  └── User profile photos (max 5MB, JPEG/PNG/GIF/WEBP)
```
...
**Helper Functions:**
```typescript
resolveMemberRole(project, userId) → MemberRole
canEditProjectContent(role) → boolean  // false for "viewer"
```
...
```typescript
toDateString(value?: Timestamp | string) → string
...
### CSV Export ([utils.ts](src/lib/utils.ts))
```typescript
exportTasksToCsv(tasks: Task[], members: UserProfile[], filename: string)
// Exports tasks sorted by order field to CSV file
// Columns: title, label, description, priority, assignees, completed, status
...
**Progress Mapping:**
- `"complete"`, `"done"` → `{status: "Done", completed: true}`
- `"review"` → `{status: "Review", completed: false}`
- `"develop"`, `"progress"` → `{status: "In Progress", completed: false}`
- `"ready"`, `"todo"` → `{status: "Backlog", completed: false}`
...
- [ ] In-app notifications (due soon, mentions, status changes)
- [ ] Activity log and comments per task
- [x] CSV export functionality (milestone tasks export)
